...
static struct str_buf sbuf_rtdb[1];
#define sbuf sbuf_rtdb
...
extern char *g_progname;   /* name of program using this module */
...
char **lrgintflg;
   ...
   *lrgintflg = NULL;
   ...
   if (strcmp(s, DVersion) != 0) {
      msg_buf = alloc(35 + strlen(s) + strlen(g_progname) + strlen(DVersion));
      sprintf(msg_buf, "found version %s, %s requires version %s",
	   s, g_progname, DVersion);
      db_err1(1, msg_buf);
      }
...
char *db_string()
   ...
   if (c == '\n')
      ++dbline;
   return str_install(sbuf); /* put string in string table */
   }

/*
		...
		*/
	    ...
	    case 'o':
		  ...
		  while (c != EOF && !C_isspace(c)) {
	       ...
	       il->u[1].s = str_install(sbuf); /* non-quoted values */
	       break;
...
int num_cases;
   {
   int c, *typ_vect, i, j, num_types, indx;
