...
struct token      *copy_t_ex (struct token *t, char *fname, int line);
void            err1         (char *s);
...
void            include      (struct token *trigger, char *fname, int system, int start);
void	init_files	(char *opt_lst,char * *opt_args);
void            init_macro   (void);
void            init_preproc (char *opt_lst, char **opt_args);
void            init_sys     (char *fname, int argc, char *argv[]);
void            init_tok     (void);
void            finish_tok   (void);
void            finish_preproc  (void);
...
void            push_src     (int kind, union src_ref *ref, struct search_dir *orig);
