...
struct node *n;
    ...
    */
   ntend = 0;
   for (sym = g_dcl_stk->tended; sym != NULL; sym = sym->u.tnd_var.next)
      ++ntend;
   if (ntend == 0)
      cur_impl->tnds = NULL;
   else
    ...
    *  table when computing the offset and g_il_indx initially contains
    *  their number.
    */
   for (sym = g_dcl_stk->tended; sym != NULL; sym = sym->u.tnd_var.next) {
    ...
    */
   nvars = -g_il_indx;  /* pre-subtract preceding number of entries */
   for (sym = g_decl_lst; sym != NULL; sym = sym->u.declare_var.next)
      sym->il_indx = g_il_indx++;
   nvars += g_il_indx;  /* compute number of entries in this part of table */
	 ...
	 switch (t->tok_id) {
		...
		*/
		  ...
		  fprintf(stderr, "%s: file %s, line %d, warning: ",
		     g_progname, src->tok->fname, src->tok->line);
...
struct node *n;
   ...
   switch (n->nd_id) {
	...
	break;
	 ...
	 sym = n->u[1].sym;
	 if (sym != NULL)
	    unuse(g_tend_lst, sym->nest_lvl - 1);
	 break;
      case TrnryNd:
	 lcl_tend(n->u[0].child);
	 lcl_tend(n->u[1].child);
...
struct node *n;
    ...
    *  are initialized.
    */
   g_n_tmp_str = 0;  /* number of temporary string buffers neeeded */
   g_n_tmp_cset = 0; /* number of temporary cset buffers needed */
   g_nxt_sbuf = 0;   /* next string buffer index; used in code generation */
   g_nxt_cbuf = 0;   /* next cset buffer index; used in code generation */
   ...
   cname = salloc(makename(buf, sizeof(buf), SourceDir, buf1, CSuffix));
   ...
   switch (fnc_ret) {
	 ...
	 break;
      case RetNoVal:
	 fprintf(g_out_file, "void ");
	 break;
      }
   fprintf(g_out_file, " %c%c%c%c_%s(", uc_letter, prfx1, prfx2, prfx3,
	cur_impl->name);
   g_fname = cname;
   g_line = 7;
   ...
   if (fall_thru) {
	 ...
	 ForceNl();
       ...
       *  within the function.
       */
      il->u[5].n = g_n_tmp_str;
      il->u[6].n = g_n_tmp_cset;
      g_n_tmp_str = 0;
      g_n_tmp_cset = 0;
      }
