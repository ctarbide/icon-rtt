...
word g_lbl_num = 0;                 /* next unused label number */
struct lvl_entry *g_dcl_stk;        /* stack of declaration contexts */
char *g_str_rslt;                     /* string "result" in string table */
struct init_tend *g_tend_lst = NULL;  /* list of tended descriptors */
struct sym_entry *g_decl_lst = NULL;  /* declarations from "declare {...}" */
struct sym_entry *g_v_len = NULL;     /* entry for length of varargs */
int g_il_indx = 0;                    /* data base symbol table index */
...
void init_sym()
   ...
   if (first_time) {
	 ...
	 sym_tbl[i] = NULL;
      g_dcl_stk = NewStruct(lvl_entry);
      g_dcl_stk->nest_lvl = 1;
      g_dcl_stk->next = NULL;
...
int lvl_incr;
   ...
   entry->nest_lvl = g_dcl_stk->nest_lvl + lvl_incr;
   entry->kind_dcl = OtherDcl;
   entry->parms_done = 0;
   entry->tended = NULL;
   entry->next = g_dcl_stk;
   g_dcl_stk = entry;
   }

/*
...
void pop_cntxt()
    ...
    */
   old_lvl = g_dcl_stk->nest_lvl;
   entry = g_dcl_stk;
   g_dcl_stk = g_dcl_stk->next;
   free((char *)entry);
...
int lvl;
   ...
   if (g_tend_lst == NULL)
      ...
      tnd->t_indx = g_tend_lst->t_indx + 1;
   tnd->init_typ = typ;
   /*
...
void free_tend()
   {
   struct init_tend *tnd, *tnd1;

   for (tnd = g_tend_lst; tnd != NULL; tnd = tnd1) {
      tnd1 = tnd->next;
      free((char *)tnd);
      }
   g_tend_lst = NULL;
...
void strt_def()
   ...
   push_cntxt(1);
   sym = sym_add(Identifier, g_str_rslt, RsltLoc, g_dcl_stk->nest_lvl);
   sym->u.referenced = 0;
   }

/*
...
struct node *dcltor;
   ...
   for (;;) {
	 ...
	 case PrimryNd:
		...
		*  context.
		*/
	       if (g_dcl_stk->kind_dcl == IsTypedef)
		  tok_id = TypeDefName;
	       else
		  tok_id = Identifier;
	       sym = sym_add(tok_id, t->image, OtherDcl, g_dcl_stk->nest_lvl);
	       if (sym == NULL)
...
struct node *init;
   ...
   switch (g_dcl_stk->kind_dcl) {
	  ...
	  */
	 sym = sym_add(Identifier, chld1->tok->image, g_dcl_stk->kind_dcl,
	    g_dcl_stk->nest_lvl);
	 if (sym == NULL)
	    errt2(chld1->tok, "redefinition of ", chld1->tok->image);
	 sym->u.tnd_var.blk_name = g_dcl_stk->blk_name;
	 sym->u.tnd_var.init = init;
	 sym->t_indx = alloc_tnd(g_dcl_stk->kind_dcl, init, g_dcl_stk->nest_lvl);
	 sym->u.tnd_var.next = g_dcl_stk->tended;
	 g_dcl_stk->tended = sym;
	 ++sym->ref_cnt;
	 return;
      default:
...
struct node *head;
   ...
   g_dcl_stk->parms_done = 0;
   if (g_dcl_stk->next->kind_dcl == IsTypedef)
...
struct token *d_ident;
   ...
   if (g_params == NULL)
      ...
      param_num = g_params->u.param_info.param_num + 1;
   if (u_ident != NULL)
...
int flags;
   ...
   sym = sym_add(Identifier, ident->image, flags, g_dcl_stk->nest_lvl);
   if (sym == NULL)
   ...
   sym->u.param_info.next = g_params;
   sym->il_indx = g_il_indx++;
   g_params = sym;
   ++sym->ref_cnt;
   }

   ...
   for (;;) {
	 ...
	 case PrimryNd:
		...
		*/
	       ...
	       ++sym->ref_cnt;
	       sym->u.declare_var.next = g_decl_lst;
	       g_decl_lst = sym;
	       }
	    return;
	 default:
...
void tnd_char()
   {
   g_dcl_stk->kind_dcl = TndStr;
   g_dcl_stk->blk_name = NULL;
   }

/*
...
struct token *t;
   ...
   if (strct_nm == descrip) {
      g_dcl_stk->kind_dcl = TndDesc;
      g_dcl_stk->blk_name = NULL;
      return;
      }
   g_dcl_stk->kind_dcl = TndBlk;
   g_dcl_stk->blk_name = strct_nm;
   }

/*
 * tnd_strct - indicate in the current declaration context that a tended
